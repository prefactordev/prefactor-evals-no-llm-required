# Prefactor Evals, No LLM required
...
There is no model client anywhere in this project and there never will be. LLM
based evals, if they ship, are a separate package, so this one cannot cost you a
token by construction.

There are two implementations, Python and TypeScript, held byte identical by a
conformance suite (see the last section). Pick the one that fits your stack; they
do the same thing with the same commands.

## Install

Nothing is published to a package index. Install from this repo.
...
That gives you `prefactor-evals-no-llm` on your path (Python), or a runnable
`dist/cli.js` (TypeScript, `npm link` it for the same name).
...
The TypeScript build takes the same commands and flags:

```
node packages/typescript/dist/cli.js run --agent <agent-id> --since 7d
```
...
`https://app.prefactorai.com`, the documented cloud host; set
`PREFACTOR_API_URL` only if your account lives somewhere else.
...
opens it. This one is Python only; the TypeScript build accepts `--html` and
notes that rather than writing pages.
...
All Prefactor coupling lives in one file per language, `source.py` and
`source.ts`. Nothing else imports it. Replace it with your own loader that emits
the documented `schema/v1` shapes and every check keeps working.

`fixtures.py` and `fixtures.ts` are working examples of exactly that: a second
source, backed by JSON files, that has never heard of Prefactor and drives the
same runner. The whole test suite runs on it, with no account and no network.
...
Both languages implement all twelve checks, including JSON Schema validation
against a bundled draft 2020-12 subset (see
[spec/shared/json-schema-subset.md](spec/shared/json-schema-subset.md)) so
neither depends on an external validator, and every one is proven conformant.
Both also have the Prefactor seam (fetching traces straight from an instance),
live mode (per-span evaluation that can stop a breaching run), and the CLI, with
the same command and flag names. The HTML dashboard is the one piece still only
in the Python build.
