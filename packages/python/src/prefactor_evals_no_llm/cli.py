...
def build_parser() -> argparse.ArgumentParser:
    ...
    run_cmd.add_argument("--pack", default=None,
                         help="Path to a pack file, or a built in name: standard, advanced. "
                              "Defaults to the standard pack, the zero config health checks "
                              "that run on any agent.")
    ...
    agents_cmd = sub.add_parser(
        "agents",
        help="List the account's agents with their ids, so you do not have to "
             "fish an id out of a dashboard URL.")
    agents_cmd.add_argument("--env-file", default=None,
                            help="Read PREFACTOR_* credentials from this file.")
    return parser
...
def _agents(args) -> int:
    """List the account's agents. The agent id is the one thing setup needs
    that nobody can type from memory, so the tool fetches it rather than
    sending people to fish it out of a dashboard URL."""
    if getattr(args, "env_file", None):
        _load_env_file(args.env_file)

    from .source import PrefactorSource, PrefactorSourceError

    try:
        agents = PrefactorSource().list_agents()
    except PrefactorSourceError as error:
        print("%s" % error, file=sys.stderr)
        return 2

    if not agents:
        print("No agents on this account yet. Instrument one with a Prefactor "
              "SDK and it will appear here.")
        return 0
    width = max(len(a["id"]) for a in agents)
    for agent in agents:
        line = "%-*s  %s" % (width, agent["id"], agent["name"])
        if agent["status"] and agent["status"] != "active":
            line += "  (%s)" % agent["status"]
        print(line)
    print("")
    print("Run evals on one with: prefactor-evals-no-llm run --agent <id> --since 7d")
    return 0
