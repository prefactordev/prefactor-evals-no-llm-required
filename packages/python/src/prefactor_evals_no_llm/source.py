    ...
    try:
        ...
        if error.code == 401:
            raise PrefactorSourceError(
                ...
                "admin API token. The SDK ingestion token used for "
                "instrumentation will not work here."
            ) from error
        raise PrefactorSourceError(
...
class PrefactorSource:
    ...
    def __init__(self, api_url: Optional[str] = None, api_token: Optional[str] = None,
                 ...
                 max_workers: int = 8):
        self.api_url = (api_url or os.environ.get("PREFACTOR_API_URL") or "").rstrip("/")
        ...
        if not self.api_url:
            # No default is guessed. Two different hosts are in circulation and
            # picking one produces confusing 401s against the wrong service.
            raise PrefactorSourceError(
                "PREFACTOR_API_URL is not set. Set it to your Prefactor API host."
            )
        if not self.api_token:
            raise PrefactorSourceError("PREFACTOR_API_TOKEN is not set.")
...
class PrefactorController:
    ...
    def __init__(self, api_url: Optional[str] = None, api_token: Optional[str] = None,
        ...
        self.timeout = timeout
        if not self.api_url:
            raise PrefactorSourceError("PREFACTOR_API_URL is not set.")
        if not self.api_token:
            raise PrefactorSourceError("PREFACTOR_API_TOKEN is not set.")
...
def declare_quality_schema(agent_id: str, version: str = "agent-evals",
    ...
    url = (api_url or os.environ.get("PREFACTOR_API_URL") or "").rstrip("/")
