...
# ---------------------------------------------------------------------------
# The bundled validator refuses what it cannot honour, rather than skip it
# ---------------------------------------------------------------------------

def test_output_schema_refuses_unsupported_keyword():
    # anyOf is a real keyword whose error selection differs between validators,
    # so it is refused by name rather than silently validated as if absent,
    # which would report a pass that checked less than the author wrote.
    schema = {"anyOf": [{"type": "object"}, {"type": "string"}]}
    root = span("r", parent_id=None, output={})
    result = output_schema.run(instance([root], output={}), {"schema": schema}, ctx())
    assert result.status == "skip"
    assert "anyOf" in result.details


def test_tool_arg_schema_refuses_tuple_items():
    schema = {"type": "array", "items": [{"type": "string"}]}
    spans = [span("create_ticket", input=["a"])]
    result = tool_arg_schema.run(instance(spans),
                                 {"schemas": {"create_ticket": schema}}, ctx())
    assert result.status == "skip"
    assert "tuple" in result.details


def test_output_schema_refuses_local_ref():
...
def test_output_schema_refuses_unportable_pattern():
    # (?i) compiles in Python and throws in JavaScript. Validating it here while
    # the other language errors would split the verdict, so it is refused.
    schema = {"type": "object",
              "properties": {"a": {"type": "string", "pattern": "(?i)yes"}}}
    root = span("r", parent_id=None, output={"a": "YES"})
    result = output_schema.run(instance([root], output={"a": "YES"}),
                               {"schema": schema}, ctx())
    assert result.status == "skip"
    assert "inline flag" in result.details


def test_schema_pattern_classes_are_ascii():
    # \d is Unicode aware in Python and ASCII only in JavaScript. The validator
    # compiles with re.ASCII so both languages agree: Arabic-Indic digits do not
    # satisfy \d, exactly as in JavaScript.
    from prefactor_evals_no_llm._schema_min import iter_errors
    schema = {"type": "string", "pattern": r"^\d+$"}
    assert iter_errors(schema, "123") == []
    assert len(iter_errors(schema, "٣٤٥")) == 1
