...
2. **Base URL.** The API is served from `https://app.prefactorai.com`, the
   host the platform docs and the OpenAPI spec publish, verified against the
   live service for reads and writes both. The seam defaults to it, and
   `PREFACTOR_API_URL` overrides the default for an account on a different
   host.
